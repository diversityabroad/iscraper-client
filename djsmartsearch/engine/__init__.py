...
logger = logging.getLogger(
    getattr(settings, 'SMARTSEARCH_LOGGER', 'djsmartsearch'))
...
class SearchEngineBase(object):
    ...
    def get_iteration_root(self):
        ...
        raise NotImplementedError(
            "implement this get_iteration_root method in a subclass")
    ...
    def parse_row(self, row):
        ...
        """
        raise NotImplementedError(
            "implement this parse_row method in a subclass")
    ...
    def set_meta_from_response(self, response):
        ...
        """
        meta = {'start_index': 0, 'end_index': 0, 'count': 0,
                'total_results': 0, 'page': 0,
    ...
    def fetch(self,  *args, **kwargs):
        ...
        response. This returns a raw search result response from the search
        engine.
...
class SmartSearchConfig(site_config.SiteConfigBase):
    ...
    def get_default_configs(self):
        ...
        SMARTSEARCH_AVAILABLE_ENGINES = getattr(
            settings, 'SMARTSEARCH_AVAILABLE_ENGINES')
        if not isinstance(SMARTSEARCH_AVAILABLE_ENGINES, dict):
            raise exceptions.ImproperlyConfigured(
                "SMARTSEARCH_AVAILABLE_ENGINES dictionary must "
                "be set in settings.py")
        elif not all(map(lambda x: x[1].has_key("CLASS"),
                         SMARTSEARCH_AVAILABLE_ENGINES.items())):
            raise exceptions.ImproperlyConfigured(
                "Each dictionary in settings.SMARTSEARCH_AVAILABLE_ENGINES "
                "must have a 'CLASS' key.")
