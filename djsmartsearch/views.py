...
class DualSearchView(SearchView):
    ...
    def form_valid(self, form):
        ...
        if self.query:
            ...
            local_kwargs = {'query': "site:%s %s" % (link_site, self.query),
                            'page': self.page_local}
            results_key = "results" + ":".join(
                map(lambda x: "%s" % x, local_kwargs.values()))
            self.results['local'], self.meta['local'] = \
                self.get_results(results_key, local_kwargs)

            global_kwargs = {'query': self.query, 'page': self.page}
            results_global_key = "results_global" + ":".join(
                map(lambda x: "%s" % x, global_kwargs.values()))
            self.results['global'], self.meta['global'] = \
                self.get_results(results_global_key, global_kwargs)
